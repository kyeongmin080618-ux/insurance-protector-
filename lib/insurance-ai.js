...
export const MODEL = process.env.GOOGLE_MODEL || process.env.GEMMA_MODEL || 'gemini-2.5-flash-lite';
...
const NO_THOUGHT_MODE_SEQUENCE = `<|turn>user
[Prompt]<turn|>
<|turn>model
<|channel>thought
<channel|>`;
...
- 다음 추론 사고모드 마커는 출력하지 말고 비활성화된 것으로 간주하십시오: ${NO_THOUGHT_MODE_SEQUENCE}
- 영어는 쓰지 말고 오직 한국어로만 최종 답변을 출력하십시오.
- "Basic Contract", "Conclusion on Payout", "One-line conclusion", "Calculation Table", "Risks", "Additional Documents", "Questions for Insurance Co", "Ensure", "No English" 같은 영어 초안/검수 메모는 절대 출력하지 마십시오.

출력 형식:
- 반드시 첫 줄을 "최종 답변"으로 시작하십시오.
- "최종 답변" 앞에는 제목, 역할 설명, 영어 요약, 지시사항, 분석 메모를 절대 쓰지 마십시오.
...
반드시 "최종 답변"으로 시작하고, 영어 제목/영어 검수 메모/프롬프트/지시사항/내부 분석은 출력하지 마세요.`;
...
function stripThinkingProcess(rawText) {
  if (!rawText) return '';
  let cleaned = rawText.replace(/<\|turn>user[\s\S]*?\[Prompt\]<turn\|>\s*<\|turn>model\s*<\|channel>?thought\s*<channel\|>/g, '');
...
function buildUpstreamErrorMessage(data, status) {
  ...
  if (lowerMessage.includes('internal error encountered') || status >= 500) {
    return `${message} 현재 모델(${MODEL})에서 일시적인 내부 오류가 발생했거나 업로드한 PDF/이미지 입력을 처리하지 못했을 수 있습니다. 환경 변수 GOOGLE_MODEL 또는 GEMMA_MODEL을 설정했다면 Google Generative Language API에서 사용할 수 있는 모델 ID인지 확인한 뒤 서버를 재시작/재배포해 주세요.`;
  }
...
const INTERNAL_ENGLISH_LABEL_PATTERNS = [
  /\bBasic Contract\b/i,
  /\bConclusion on Payout\b/i,
  /\bOne-line conclusion\b/i,
  /\bCalculation Table\b/i,
  /\bCancer Diagnosis Fee\b/i,
  /\bPremium Waiver\b/i,
  /\bTargeted Therapy\b/i,
  /\bHeavy Ion Therapy\b/i,
  /\bCAR-T Therapy\b/i,
  /\bCancer Major Treatment Fee\b/i,
  /\bDiagnosis alone\b/i,
  /\bTreatment\s*→/i,
  /\bConfidence:\s*(High|Medium|Low)\b/i,
  /\bWaiting period\b/i,
  /\bReduction:\s*\d+%/i,
  /\bTreatment-specific\b/i,
  /\bRisk Factors?\b/i,
  /\bRisks?\b/i,
  /\bAdditional Documents?\b/i,
  /\bQuestions for Insurance Co\b/i,
  /\bKRW notation\b/i,
  /\bNo English\b/i,
  /\bAvoid definitive words?\b/i,
  /\bEnsure\s+["“]?최종 답변/i,
];
...
function hasInternalEnglishLabel(text) {
  return INTERNAL_ENGLISH_LABEL_PATTERNS.some((pattern) => pattern.test(text));
}
...
function looksLikeInternalInstruction(line) {
  ...
  if (hasInternalEnglishLabel(trimmed)) return true;
...
function removeInternalInstructionLines(text) {
  return text
    .split(/\r?\n/)
...
function fallbackForInternalOutput() {
  return `${FINAL_ANSWER_MARKER}\n모델이 영어 초안 또는 내부 검수 메모를 반환해 결과를 표시하지 않았습니다. 다시 분석을 실행해 주세요.`;
}
...
function sanitizeModelOutput(text) {
  ...
  if (markerIndex !== -1) {
    const finalAnswer = removeInternalInstructionLines(withoutThinking.slice(markerIndex));
    if (!finalAnswer || hasInternalEnglishLabel(finalAnswer)) return fallbackForInternalOutput();
    return finalAnswer;
  }

  if (hasInternalEnglishLabel(withoutThinking) && !hasKorean(withoutThinking.replace(FINAL_ANSWER_MARKER, ''))) {
    return fallbackForInternalOutput();
  }
  ...
  const cleaned = removeInternalInstructionLines(candidateLines.join('\n'));
  if (!cleaned || hasInternalEnglishLabel(cleaned)) return fallbackForInternalOutput();
