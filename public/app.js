...
async function readJsonResponse(response) {
  ...
  try {
    ...
    const deploymentHint = compact.includes('SERVICE_UNAVAILABLE') || compact.includes('deployment is currently unavailable')
      ? ' Vercel 배포가 실패했거나 서버리스 API가 준비되지 않았습니다. 배포 로그와 GOOGLE_API_KEY 환경 변수를 확인한 뒤 재배포해 주세요.'
      : '';
    throw new Error(`서버가 JSON이 아닌 응답을 반환했습니다. 현재 앱 서버(/api)가 실행 중인지 확인해 주세요.${deploymentHint} 상태: ${response.status} ${response.statusText}. 응답: ${preview}`);
  }
}
